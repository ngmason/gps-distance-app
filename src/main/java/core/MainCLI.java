...
public class MainCLI {
    ...
    public static void gpsAppTextConsole() {
        try (SQLiteRouteRepository repo = new SQLiteRouteRepository(AppPaths.resolveDbPath())) {

            try {
                savedRoutes = new ArrayList<>(repo.loadRoutes());
            } catch (SQLException e) {
                System.err.println("Could not load saved routes: " + e.getMessage());
                return;
            }
            ...
            do {
                ...
                Route newRoute = new Route(coordinate1, coordinate2, AVERAGE_SPEED, nextRouteName(savedRoutes));
                try {
                    repo.saveRoute(newRoute);
                    savedRoutes.add(newRoute);
                } catch (SQLException e) {
                    System.err.println("Warning: could not save route: " + e.getMessage());
                }
            ...
            System.err.println("Could not open route database: " + e.getMessage());
        }
    }

    private static String nextRouteName(ArrayList<Route> routes) {
        for (int n = 1; ; n++) {
            final String candidate = "Route " + n;
            if (routes.stream().noneMatch(r -> r.getName().equalsIgnoreCase(candidate))) {
                return candidate;
            }
        }
    }
