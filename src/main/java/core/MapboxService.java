...
public class MapboxService {
     ...
     */
    public String getEncodedPolyline(List<Location> locations) throws Exception {
        if (locations.size() < 2) throw new IllegalArgumentException("At least 2 locations required");
        StringBuilder coords = new StringBuilder();
        for (int i = 0; i < locations.size(); i++) {
            if (i > 0) coords.append(";");
            Location loc = locations.get(i);
            coords.append(String.format("%f,%f", loc.getLongitude(), loc.getLatitude()));
        }
        String urlStr = BASE_URL + coords + "?geometries=polyline&overview=simplified&access_token=" + token;
    ...
    public String getEncodedPolyline(double lonA, double latA, double lonB, double latB) throws Exception {
        return getEncodedPolyline(List.of(new Location("", latA, lonA), new Location("", latB, lonB)));
    }
     ...
     */
    public String buildStaticMapUrl(String encodedPolyline, List<Location> locations,
                                    double centerLon, double centerLat, int zoom) {
        StringBuilder overlays = new StringBuilder();
        overlays.append("path-5+ff0000-0.8(").append(encodedPolyline).append(")");
        for (int i = 0; i < locations.size(); i++) {
            Location loc = locations.get(i);
            String color = (i == 0) ? "ff0000" : (i == locations.size() - 1) ? "0000ff" : "ff8c00";
            overlays.append(String.format(",pin-s+%s(%f,%f)", color, loc.getLongitude(), loc.getLatitude()));
        }
        return String.format("%s%s/%f,%f,%d/600x400?access_token=%s",
            STYLE_URL, overlays, centerLon, centerLat, zoom, token);
    }
    ...
    public String buildStaticMapUrl(String encodedPolyline,
        ...
        return buildStaticMapUrl(
            encodedPolyline,
            List.of(new Location("", latA, lonA), new Location("", latB, lonB)),
